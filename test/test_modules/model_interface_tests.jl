@testmodule ModelInterfaceTests begin
    using Test
    using GraphPPL

    # Define a placeholder for fform for testing factor creation
    struct TestFactorForm end

    """
        model_interface_test_suite(model_type::Type{<:ModelInterface})

    Provides a comprehensive test suite for implementations of ModelInterface.

    # Arguments
    - `model_type`: The concrete model type that implements ModelInterface

    # Example
    ```julia
    # Tests for the default model implementation
    @testitem "Default model implementation tests" begin
        ModelInterfaceTests.model_interface_test_suite(
            GraphPPL.Model
        )
    end

    # Tests for a custom model implementation
    @testitem "Custom model implementation tests" begin
        ModelInterfaceTests.model_interface_test_suite(
            MyCustomModel
        )
    end
    """
    function model_interface_test_suite(model_type::Type{<:ModelInterface})
        @testset "Variable node operations" begin
        ...
        @testset "Factor node operations" begin
            # Create instance of model
            model = create_model(model_type)
        ...
        @testset "Neighbor operations" begin
            model = create_model(model_type)
            context = get_context(model)
        ...
        @testset "Model save and load" begin
            model = create_model(model_type)
            context = get_context(model)
    ...
    export model_interface_test_suite
end # module ModelInterfaceTests
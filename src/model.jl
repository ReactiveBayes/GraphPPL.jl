...
function generate_model_expression(backend, model_specification)
    ...
    ms_body = prewalk(ms_body) do expression
        ...
        elseif @capture(expression, varexpr_ = randomprocess(arguments__) where { options__ })
            return :($varexpr = randomprocess($(arguments...); $(options...)))   #add randomprocess here 
        ...
        elseif @capture(expression, varexpr_ = randomprocess(arguments__))  #add randomprocess here 
    ...
    ms_body = postwalk(ms_body) do expression
        ...
        elseif @capture(expression, varexpr_ = randomprocess(arguments__; options__))
            rpoptions = write_randomprocess_options(backend, varexpr, options)
            return write_randomprocess_expression(backend, model, varexpr, rpoptions, arguments)

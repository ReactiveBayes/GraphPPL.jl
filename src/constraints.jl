...
function generate_constraints_expression(
    backend,
    constraints_options,
    ...
    if isblock(constraints_specification)
        ...
        return :($(generate_constraints_expression(
            backend,
            constraints_options,
            generatedfbody,
        ))())
    ...
    cs_body = prewalk(cs_body) do expression
        if iscall(expression, :(::))
            if @capture(expression.args[2], q(formsym_Symbol))
                specs = map(
                    (e) -> parse_form_constraint(backend, e),
                    view(expression.args, 3:lastindex(expression.args)),
                )
                ...
                return quote
                    ...
                    $marginals_form_constraints_symbol =
                        (; $marginals_form_constraints_symbol..., $formsym = $form)
                end
            elseif @capture(expression.args[2], μ(formsym_Symbol))
                ...
                return quote
                    ...
                    $messages_form_constraints_symbol =
                        (; $messages_form_constraints_symbol..., $formsym = $form)
                end
            else
                error(
                    "Invalid form factorisation constraint. $(expression.args[2]) has to be in the form of q(varname) for marginal form constraint or μ(varname) for messages form constraint.",
                )
            end
        end
        return expression
    ...
    cs_body = postwalk(cs_body) do expression
        ...
        if @capture(expression, lhs_ = rhs_) && @capture(lhs, q(names__))
            ...
            (length(names) !== 0 && all(name -> name isa Symbol, names)) || error(
                """Error in factorisation constraints specification $(lhs_name) = ...\nLeft hand side of the equality expression should have only variable identifiers.""",
            )
                ...
                if @capture(entry, q(indices__))
                    for index in indices
                        if index isa Symbol
                            ...
                            (index ∉ lhs_names) && error(
                                "LHS of the $(expression) expression does not have $(index) variable, but is used in RHS.",
                            )
                        elseif isref(index)
            ...
            (lhs_names == rhs_names) || error(
                "LHS and RHS of the $(expression) expression has different set of variables.",
            )
            ...
            lhs_info = if haskey(lhs_dict, lhs_hash)
                ...
                lhs_info =
                    FactorisationConstraintLHSInfo(lhs_name, lhs_hash, lhs_varname)
            ...
            new_factorisation_specification = write_factorisation_constraint(
                backend,
                :(Val(($(map(QuoteNode, names)...),))),
                :(Val($(rhs))),
            )
            check_is_not_defined =
                write_check_factorisation_is_not_defined(backend, lhs_varname)

            result = quote
                $(check_is_not_defined) || error(
                    "Factorisation constraints specification $($lhs_name) = ... has been redefined.",
                )
                $(lhs_varname) = $(new_factorisation_specification)
                $factorisation_constraints_symbol =
                    ($factorisation_constraints_symbol..., $(lhs_varname))
            end
        ...
        if @capture(expression, q(args__))
            ...
            rhs_prod_entries_args = map(args) do arg
                if arg isa Symbol
                    ...
                    return :(nothing)
                elseif isref(arg)
                    (length(arg.args) === 2) || error(
                        "Indexing expression $(expression) is too difficult to parse and is not supported (yet?).",
                    )
                    ...
                    index = postwalk(index) do iexpr
                        if iexpr isa Symbol && iexpr === :begin
                            return write_factorisation_functional_index(
                                backend,
                                :begin,
                                :firstindex,
                            )
            ...
            entry = write_factorisation_constraint_entry(
                backend,
                :(Val(($(map(QuoteNode, rhs_prod_names)...),))),
                :(Val(($(rhs_prod_entries_args...),))),
            )

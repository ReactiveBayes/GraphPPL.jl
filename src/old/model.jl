...
function conditioned_walk(f, condition_skip, condition_apply, x)
    walk(x, x -> if condition_skip(x)
        x
    elseif condition_apply(x)
        f(x)
    else
...
function generate_model_expression(backend, model_specification)
    ...
    # Step 5: Final pass
    final_pass_exceptions = (x) -> @capture(x, (some_ -> body_) | (function some_(args__)
        body_
    ...
    ms_structure = write_model_structure(backend, ms_name, model, ms_args_checks, ms_args_const_init_block, ms_args, ms_kwargs, ms_body)

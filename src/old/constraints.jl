...
function generate_constraints_expression(backend, constraints_options, constraints_specification)
    ...
    @capture(constraints_specification, (function cs_name_(cs_args__; cs_kwargs__)
        cs_body_
    end) | (function cs_name_(cs_args__)
        cs_body_
    end)) || error("Constraints specification language requires full function definition")
    ...
    return_specification = write_constraints_specification(
        backend, factorisation_constraints_symbol, marginals_form_constraints_symbol, messages_form_constraints_symbol, cs_options
    )
